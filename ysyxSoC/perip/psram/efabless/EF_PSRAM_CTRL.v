...
module PSRAM_QPI_ENABLER (
    ...
    always @*
        case (state)
            IDLE: if(start) nstate = SEND; else nstate = IDLE;
            SEND: if(done) nstate = IDLE; else nstate = SEND;
        endcase
    ...
    assign dout = (counter < 8) ? {3'b0, CMD_ENTER_QPI[7 - counter]} : 4'h0;
    assign douten = ~ce_n;
...
module PSRAM_READER (
    ...
    wire [7:0]  FINAL_COUNT = 13 + size*2; // QPI: 2 cmd + 6 addr + 6 dummy + data
