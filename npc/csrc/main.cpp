...
// 【核心修复】：将物理内存容量扩大到 128MB，满足 RT-Thread 的默认堆栈分配需求
...
extern "C" int pmem_read(int raddr) {
    ...
    if (addr >= MEM_BASE && addr < MEM_BASE + MEM_SIZE) {
        int paddr = addr - MEM_BASE;
        uint32_t data = *(uint32_t *)(mem + paddr);
        return data;
    }
    // C++ 端保底防卫，理论上外设不会进到这里，全在 Xbar 处理
    is_skip_ref = true;
    return 0; 
}
...
void cpu_exec(uint64_t n){
    static svScope scope = svGetScopeFromName("TOP.example"); 
    svSetScope(scope);
